...
export default function How({ isOpen, onClose }) {
    ...
    useEffect(() => {
        if (isOpen) {
            setVisible(true);
            setTimeout(() => setAnimate(true), 50); // Slight delay to apply transition classes
        } else {
            setAnimate(false);
    ...
    return ReactDOM.createPortal(
        <div className={`fixed inset-0 bg-black bg-opacity-[0.35] flex justify-center items-center z-50 transition-opacity duration-300 ${animate ? 'opacity-100' : 'opacity-0'}`}>
            <div className={`bg-con-200 px-7 pb-4 pt-7 rounded-[10px] border-[3px] border-con-900 relative max-w-[480px] sm:w-[480px] md:w-[480px] lg:w-[480px] mx-[15px] transform transition-transform duration-300 ${animate ? 'translate-y-0' : isOpen ? '-translate-y-full' : 'translate-y-full'}`}>
